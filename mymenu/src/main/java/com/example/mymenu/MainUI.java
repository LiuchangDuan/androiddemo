...
public class MainUI extends RelativeLayout {
    ...
    private float onMiddleMask() {
        System.out.println("Alpha : " + middleMask.getAlpha());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int tempWidthMeasure =
                MeasureSpec.makeMeasureSpec((int)(realWidth * 0.8f), MeasureSpec.EXACTLY);
            ...
            switch (ev.getActionMasked()) {
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    if (Math.abs(curScrollX) > leftMenu.getMeasuredWidth() >> 1) {
                        if (curScrollX < 0) { // 向左
                            mScroller.startScroll(curScrollX, 0, -leftMenu.getMeasuredWidth() - curScrollX, 0, 200); // 最后一个参数是时长
                        } else { // 向右
                            mScroller.startScroll(curScrollX, 0, leftMenu.getMeasuredWidth() - curScrollX, 0, 200);
                        }
                    } else {
                        mScroller.startScroll(curScrollX, 0, -curScrollX, 0, 200); // 滚回去
                    }
                    invalidate();
